...
bool level_1(char *arr)
{
    int count_low = 0;
    int count_caps = 0;
    //Prochazeni hesla po znaku až do '\0'
        ...
        if (first_index == param)
        {
            ...
            for (int y = bump + 1; second_index < param && arr[y] != '\0'; y++)
            {
                ...
                if (second_index == param)
                {
                    ...
                    //(př.: parametr = 3, compare_first bude obsahovat znaky na indexu [O],[1],[2] a compare_second znaky na indexu [1],[2],[3])
...
{
    (void)argc; //counter argumentů
    (void)argv; //argument

    int level;          //level
    long param;         //parametr
    char password[102]; //string pro uložení hesla

    //Proměnné pro statistiku
    ...
    //Pokud je argc vetsi jak dva znamena to, ze obsahuje minimálne nazev, level a parametr
    if (argc > 2)
    {
        ...
        for (int i = 1; i < argc; i++)
        {
            ...
            //pokud je hodnota promenne param mensi nebo rovna nule, tak se vyhodí error
            if (i == 2)
            {
                param = atoi(argv[2]);
                if (param <= 0)
            ...
            //na vstupu rovna stringu stats pokud ano tak se bool statistics zmeni na true a později se vytisknou. Pokud ne tak se hodi error - chybne zapsano
            if (i == 3)
            {
                char *stats = "--stats";
            ...
            if (i >= 4) //Pokud je na vstupu vice parametru nez muže byt vyhodi se error
            {
    ...
    while (fgets(password, 102, stdin) != NULL)
    {
        ...
        for (int i = 0; password[i] != '\0' && password[i] != '\n'; i++)
        {
            ...
            compare = password[i];
            //pomocná promenna pro urceni zda se znak nachazi v poli unique_chars
            int flag = 1;
